...
class FraudDatabase:
        ...
        try:
            ...
            cursor.execute("""
                ...
                (id, userName, securityIdentifier, cardEnding, cardType, 
                 ...
                 status, securityQuestion, securityAnswer, createdAt, lastUpdated)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                ...
                case.securityAnswer, case.createdAt, datetime.now().isoformat()
            ))
    ...
    def update_fraud_case_status(self, case_id: str, status: str, outcome: str, note: str) -> bool:
        ...
        try:
            ...
            conn.commit()
            conn.close()
            print(f"✅ Updated fraud case {case_id}: status={status}, outcome={outcome}")
            return True
        except Exception as e:
    ...
    def import_from_json(self, input_file: str) -> bool:
        ...
        try:
            ...
            print(f"✅ Imported {len(data.get('fraud_cases', []))} cases from {input_file}")
            return True
        except Exception as e:
